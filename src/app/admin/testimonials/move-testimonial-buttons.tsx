...
import { moveTestimonial } from '@/server/actions/testimonials';

/**
 * Simple up/down reordering, not drag-and-drop - each click updates
 * `display_order` server-side (see moveTestimonial) and the list
 * re-renders via revalidatePath. Disabled at the top/bottom of the list
 * instead of wrapping around.
 */
export function MoveTestimonialButtons({
  id,
  disableUp,
  disableDown,
}: {
  ...
  function handleMove(direction: 'up' | 'down') {
    startTransition(() => {
      void moveTestimonial(id, direction);
    });
  }
